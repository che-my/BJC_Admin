...
var handleSidebarMinify = function() {
	$(document).on('click', '[data-click=sidebar-minify]', function(e) {
		...
		if ($(targetContainer).hasClass(sidebarClass)) {
			// 展开
			$(targetContainer).removeClass(sidebarClass);
			// $('.top-menu').css({"position":"relative","left":"60px","top":"50px"});
		} else {
			// 收缩
			$(targetContainer).addClass(sidebarClass);
			if(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
				...
				// $('#setting [data-scrollbar="true"]').css('margin-top','0');
				// $('#setting [data-scrollbar="true"]').css('overflow-x', 'scroll');
			}
			// $('.top-menu').css({"position":"relative","left":"220px","top":"50px"});
		}
		var left = $('.top-menu').css('left');
		console.log(left)
		// if(left=='220px'){
		// 	console.log(left)
		// 	$('.top-menu').css({"position":"fixed","left":"60px"});
		// }else{
		// 	$('.top-menu').css({"position":"fixed","left":"220px"});
		// }
		$(window).trigger('resize');
	});
};
		...
		if ($(this).val() == 1) {
			...
			if ($('.sidebar .slimScrollDiv').length !== 0) {
				if ($(window).width() <= 979) {
					...
					$('#setting [data-scrollbar="true"]').slimScroll({destroy: true});
...
var handleResetLocalStorage = function() {
	...
	$(document).on('click', '[data-click=reset-local-storage]', function(e) {
		...
		'                <h4 class="modal-title"><i class="fa fa-redo m-r-5"></i>重置主题设置</h4>'+
		'                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>'+
		'            </div>'+
		'            <div class="modal-body">'+
		'                <div class="alert alert-info m-b-0">请确定是否需要重置主题设置?</div>'+
		'            </div>'+
		'            <div class="modal-footer">'+
		'                <a href="javascript:;" class="btn btn-sm btn-default" data-dismiss="modal"><i class="fa fa-times"></i> 否</a>'+
